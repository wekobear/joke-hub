...
function eqOp(v) {
  // 裸顶层过滤的值按字面量解析（官方文档 Horizontal Filtering）：加引号会让
  // PostgREST 把引号当值的一部分（eq."x" ≠ x），导致详情/date 等查不到。
  // 双引号转义只在 or/and/in 括号上下文内生效（见 Logical operators / in 操作符），
  // 那些场景分别走 or 模板字符串与 inOp；&、#、引号等特殊字符由 qs() 的
  // URLSearchParams 统一做 URL 编码，无需在此转义。
  return `eq.${v}`;
}
